...
# "Nymphal habitat drydown prolongs development but preserves adult body size and
...
#   distributions are used to compare life-history traits (nymphal period,
#   preoviposition period, total number of eggs laid, and fresh body weight at
#   adult emergence) among four experimental groups (A-D) differing in nymphal
...
nymphal_data <- read_csv("Nymphal_period_data.csv")
...
# Expand frequency-format nymphal period data to individual-level data
nymphal_expanded <- nymphal_data %>%
...
# ============================================================================
# ANALYSIS 1: Nymphal Period
# GLM with Poisson distribution and log link
# ============================================================================

cat("========================================\n")
cat("ANALYSIS 1: Nymphal Period (Poisson GLM)\n")
cat("========================================\n\n")

nymphal_summary <- nymphal_expanded %>%
  group_by(Group) %>%
  summarise(
    n    = n(),
    mean = mean(Nymphal_period_days),
    SD   = sd(Nymphal_period_days),
    .groups = "drop"
  )
print(nymphal_summary)

model_nymphal <- glm(
  Nymphal_period_days ~ Group,
  data   = nymphal_expanded,
  family = poisson(link = "log")
)

cat("\nGLM Summary:\n")
print(summary(model_nymphal))

model_null_nymphal <- glm(
  Nymphal_period_days ~ 1,
  data   = nymphal_expanded,
  family = poisson(link = "log")
)

lr_nymphal <- anova(model_null_nymphal, model_nymphal, test = "Chisq")
...
emm_nymphal      <- emmeans(model_nymphal, ~ Group)
pairwise_nymphal <- contrast(emm_nymphal, method = "pairwise", adjust = "bonferroni")

cat("\nEstimated Marginal Means:\n")
print(emm_nymphal, type = "response")

cat("\nPairwise Comparisons (Bonferroni-corrected):\n")
print(pairwise_nymphal, type = "response")
...
letters_nymphal <- c(A = "a", B = "b", C = "c", D = "d")

nymphal_table <- nymphal_summary %>%
  mutate(
    letter           = letters_nymphal[as.character(Group)],
    Mean_with_letter = paste0(sprintf("%.2f", mean), letter)
  )

cat("\nSummary for Table 1 (nymphal period):\n")
print(nymphal_table[, c("Group", "n", "mean", "SD", "letter")], row.names = FALSE)
...
summary_table <- data.frame(
  ...
  P_value    = c(
    format(lr_nymphal$`Pr(>Chi)`[2], digits = 3),
...
# Survival counts per group (from nymphal period data)
survival_data <- read_csv("Nymphal_period_data.csv") %>%
  group_by(Group) %>%
